...
// import io.flutter.plugin.common.PluginRegistry.Registrar  // Deprecated
...
class FlutterNaverLoginPlugin : FlutterPlugin, MethodCallHandler, ActivityAware {
    ...
    // in the same class.
    companion object {
        // Removed the registerWith method as it's deprecated
    }
    ...
    private fun getCurrentAccessToken(result: Result) {
        println("=== getCurrentAccessToken 호출됨 ===")
        
        val currentAccessToken = NaverIdLoginSDK.getAccessToken()
        val currentRefreshToken = NaverIdLoginSDK.getRefreshToken()
        val currentExpiresAt = NaverIdLoginSDK.getExpiresAt()
        val currentTokenType = NaverIdLoginSDK.getTokenType()
        
        println("Raw AccessToken: [$currentAccessToken]")
        println("Raw RefreshToken: [$currentRefreshToken]")
        println("Raw ExpiresAt: [$currentExpiresAt]")
        println("Raw TokenType: [$currentTokenType]")
        
        val info = HashMap<String, String>()
        info["status"] = "getToken"
        println("Map after status: $info")

        if (currentAccessToken != null && currentAccessToken.isNotEmpty()) {
            println("AccessToken 존재함: [$currentAccessToken]")
            info["accessToken"] = currentAccessToken
            println("Map after accessToken: $info")
        } else {
            println("AccessToken이 null이거나 비어있음")
            info["accessToken"] = ""
            println("Map after empty accessToken: $info")
        }

        if (currentRefreshToken != null && currentRefreshToken.isNotEmpty()) {
            println("RefreshToken 존재함: [$currentRefreshToken]")
            info["refreshToken"] = currentRefreshToken
            println("Map after refreshToken: $info")
        } else {
            println("RefreshToken이 null이거나 비어있음")
            info["refreshToken"] = ""
            println("Map after empty refreshToken: $info")
        }

        info["expiresAt"] = currentExpiresAt.toString() 
        println("Map after expiresAt: $info")

        if (currentTokenType != null && currentTokenType.isNotEmpty()) {
            println("TokenType 존재함: [$currentTokenType]")
            info["tokenType"] = currentTokenType
            println("Map after tokenType: $info")
        } else {
            println("TokenType이 null이거나 비어있음")
            info["tokenType"] = ""
            println("Map after empty tokenType: $info")
        }
        
        println("최종 토큰 정보 직전: $info")
        println("최종 토큰 정보: $info")
        result.success(info)
    }
...
enum class FlutterPluginMethod(val methodName: String) {
    InitSdk("initSdk"),
    LogIn("logIn"),
    LogOut("logOut"),
