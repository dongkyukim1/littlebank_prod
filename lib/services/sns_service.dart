...
class SNSService {
  ...
  static Future<Map<String, dynamic>> loginWithKakao(String accessToken) async {
    ...
    print('카카오 로그인 API 요청: $body');
    print('카카오 로그인 API URL: $url');
    ...
    try {
      ...
      print('카카오 로그인 응답 헤더: ${response.headers}');
      print('카카오 로그인 응답 본문: ${response.body}');
      ...
      if (response.statusCode == 200) {
        ...
        print('추출된 리프레시 토큰: $refreshToken');
        ...
        try {
          ...
          // 토큰이 없는 경우 카카오 계정으로 회원가입 시도
          if (jwtToken == null || jwtToken.isEmpty) {
            print('JWT 토큰이 없어 회원가입 시도');
            try {
              ...
              print('회원가입 시도 중 오류: $signupError');
            }
          }
        }
        ...
        // 네이버 로직처럼 회원가입 시도
        return await _signupWithKakao(user: user, accessToken: accessToken);
  ...
  static Future<Map<String, dynamic>> _signupWithKakao({
    ...
    try {
      ...
      print('카카오 회원가입 응답 상태: ${response.statusCode}');
      print('카카오 회원가입 응답 헤더: ${response.headers}');

      if (response.statusCode == 200 || response.statusCode == 201) {
        // 성공
        final decodedBody = utf8.decode(response.bodyBytes);
        final result = jsonDecode(decodedBody);
        print('카카오 회원가입 응답 본문: $result');
        ...
        if (result.containsKey('accessToken') &&
          ...
          print('카카오 회원가입 후 액세스 토큰 저장: ${result['accessToken']}');
        } else {
          // 토큰이 없으면 원래 카카오 토큰을 임시로 저장
          await storage.write(
            key: AuthService.accessTokenKey,
            value: accessToken,
          );
          print('응답에 액세스 토큰이 없어 카카오 토큰 임시 저장: $accessToken');
        }

        // 현재 토큰이 만료될 수 있으므로 사용자 정보를 다시 조회
        try {
          final updatedUserInfo = await AuthService.getUserInfo();
          print('회원가입 후 사용자 정보 조회 성공: $updatedUserInfo');
          return updatedUserInfo;
        } catch (e) {
          print('회원가입 후 사용자 정보 조회 실패: $e');
          return {'role': 'CHILD', ...result};
        }
      } else if (response.statusCode == 400) {
        // 이미 가입된 이메일 등의 오류 - 로그인 시도
        final errorBody = utf8.decode(response.bodyBytes);
        print('카카오 회원가입 400 오류: $errorBody');

        try {
          // 중복 이메일은 이미 등록된 사용자이므로 바로 로그인 처리
          print('이미 등록된 사용자로 처리');
          ...
          print('카카오 토큰 저장: $accessToken');

          // 소셜 타입 저장
          await AuthService.setSocialType('KAKAO');

          // 기존 사용자 정보로 로그인 성공 처리
          return {
            'role': 'CHILD', // 기본 역할 설정
            'email': user.kakaoAccount?.email,
            'name': user.kakaoAccount?.profile?.nickname,
            'needsAdditionalInfo': true, // 추가 정보 입력 필요
          };
        } catch (e) {
          ...
          print('예외 발생으로 카카오 토큰 임시 저장: $accessToken');

          return {'role': 'CHILD', 'needsAdditionalInfo': true};
        }
      } else {
        // 기타 오류
        final errorBody = utf8.decode(response.bodyBytes);
        print('카카오 회원가입 기타 오류: ${response.statusCode}, $errorBody');
      ...
      // 마지막 대안: 임시 응답 반환
      // 카카오 토큰 임시 저장
      await storage.write(key: AuthService.accessTokenKey, value: accessToken);
      print('예외 발생으로 카카오 토큰 임시 저장: $accessToken');
  ...
  // 카카오 계정으로 로그인 (이미 가입된 경우)
  static Future<Map<String, dynamic>> _loginWithKakaoAccount({
    required String email,
    required String kakaoId,
    required String fcmToken,
    required String accessToken,
  }) async {
    print('카카오 계정으로 로그인 시도: $email');
    ...
    final Map<String, dynamic> body = {
      'email': email,
      'password': 'KAKAO_LOGIN_$kakaoId',
      'fcmToken': fcmToken,
    };

    print('카카오 계정 로그인 요청: $body');
    ...
    try {
      ...
      print('카카오 계정 로그인 응답 상태: ${response.statusCode}');
      print('카카오 계정 로그인 응답 헤더: ${response.headers}');

      if (response.statusCode == 200) {
        final decodedBody = utf8.decode(response.bodyBytes);
        final result = jsonDecode(decodedBody);
        print('카카오 계정 로그인 응답: $result');
        ...
        if (result.containsKey('accessToken') &&
          ...
          print('카카오 계정 로그인 액세스 토큰 저장: ${result['accessToken']}');
        }
        ...
        if (response.headers.containsKey('refresh-token')) {
          ...
          print('리프레시 토큰 저장: ${response.headers['refresh-token']}');
        }

        // 사용자 정보 조회로 역할 확인
        try {
          final userInfo = await AuthService.getUserInfo();
          print('카카오 계정 로그인 후 사용자 정보 조회 성공: $userInfo');
          ...
          return {...userInfo, 'needsAdditionalInfo': needsAdditionalInfo};
        } catch (e) {
          print('카카오 계정 로그인 후 사용자 정보 조회 실패: $e');
          return {...result, 'needsAdditionalInfo': true};
        }
      } else {
        final errorBody = utf8.decode(response.bodyBytes);
        print('카카오 계정 로그인 실패: ${response.statusCode}, $errorBody');
        throw Exception('카카오 계정 로그인 실패: ${response.statusCode}');
      }
    } catch (e) {
      print('카카오 계정 로그인 API 호출 예외: $e');
      throw Exception('카카오 계정 로그인 중 오류가 발생했습니다: $e');
    }
  }
  ...
  static Future<Map<String, dynamic>> setKakaoUserInfo({
    ...
    // 비밀번호 처리
    final String userPassword = password ?? 'KAKAO_LOGIN_${user.id}';

    // 필수 필드 설정
    final Map<String, dynamic> body = {
      'email': user.kakaoAccount?.email ?? 'kakao${user.id}@example.com',
      'password': userPassword, // 사용자 입력 비밀번호 또는 기본 소셜 로그인용 패스워드
  ...
  static Future<Map<String, dynamic>> _updateExistingUser({
    ...
    // 업데이트 요청 바디
    final Map<String, dynamic> body = {'rrn': birthdate, 'role': role};

    // 선택적 필드 추가
    if (name != null && name.isNotEmpty) {
      body['name'] = name;
    } else if (user.kakaoAccount?.profile?.nickname != null &&
        user.kakaoAccount!.profile!.nickname!.isNotEmpty) {
      body['name'] = user.kakaoAccount!.profile!.nickname!;
    }

    if (user.kakaoAccount?.email != null &&
        user.kakaoAccount!.email!.isNotEmpty) {
  ...
  static Future<void> _updateAdditionalUserInfo({
    ...
    if (bankName != null && bankName.isNotEmpty) {
      additionalInfo['bankName'] = bankName;
    }
    if (bankAccount != null && bankAccount.isNotEmpty) {
      additionalInfo['bankAccount'] = bankAccount;
    }
    if (bankCode != null && bankCode.isNotEmpty) {
      additionalInfo['bankCode'] = bankCode;
    }
    if (password != null && password.isNotEmpty) {
      additionalInfo['password'] = password;
    }
  ...
  static Future<Map<String, dynamic>> updateSocialAdditionalInfo({
    ...
    if (response.statusCode == 200) {
      ...
      final errorBody = utf8.decode(response.bodyBytes);
      throw Exception(
        '소셜 로그인 추가 정보 API 오류: ${response.statusCode}, $errorBody',
      );
    }
  }
  ...
  static Future<Map<String, dynamic>> updateProfileImagePath({
    ...
    final url = Uri.parse('$baseUrl/api-user/user/rrage');

    // API 스펙에 맞게 요청 바디 구성
    final Map<String, dynamic> body = {'profileImagePath': profileImagePath};
    ...
    if (response.statusCode == 200) {
      ...
      final errorBody = utf8.decode(response.bodyBytes);
      throw Exception(
        '프로필 이미지 업데이트 API 오류: ${response.statusCode}, $errorBody',
      );
    }
  }
  ...
  static Future<Map<String, dynamic>> _signupWithNaver({
    ...
    // 비밀번호 처리
    final String userPassword = password ?? 'NAVER_LOGIN_$naverAccountId';

    // 요청 바디 생성
    final Map<String, dynamic> body = {
      'email': email,
      'password': userPassword, // 사용자 입력 비밀번호 또는 소셜 로그인용 특수 패스워드
      'name': name,
      'phone': userPhone,
      'rrn': birthdate,
      'role': role,
    ...
    try {
      ...
      try {
        ...
        // 최후의 수단: 임시 응답 반환 - 반드시 추가 정보 입력 필요
        return {'role': 'CHILD', 'needsAdditionalInfo': true};
      }
    }
  }
  ...
  // 네이버 토큰으로 로그인 (백엔드)
  static Future<Map<String, dynamic>> loginWithNaverToken(
    String accessToken,
    String id,
  ) async {
    if (baseUrl.isEmpty) {
    ...
    await AuthService.setNaverAccountId(id);

    final url = Uri.parse('$baseUrl/api-user/auth/public/naver');
