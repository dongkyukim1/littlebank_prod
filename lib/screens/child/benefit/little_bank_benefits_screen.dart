...
import 'gift_detail_bottom_sheet.dart';
import 'confirmation_bottom_sheet.dart';
import 'purchase_option_bottom_sheet.dart';
import 'purchase_detail_bottom_sheet.dart';

class LittleBankBenefitsScreen extends StatefulWidget {
  const LittleBankBenefitsScreen({Key? key}) : super(key: key);

  @override
  State<LittleBankBenefitsScreen> createState() =>
      _LittleBankBenefitsScreenState();
}

class _LittleBankBenefitsScreenState extends State<LittleBankBenefitsScreen> {
  // 현재 선택된 탭 인덱스
  ...
  void _processPurchase() {
    PurchaseOptionBottomSheet.show(context, (
      title,
      price,
      persons,
      hasDiscount,
    ) {
      PurchaseDetailBottomSheet.show(
        context,
        title,
        price,
        persons,
        hasDiscount,
        (title, price, persons, hasDiscount, phoneNumbers) {
          _completePurchase(title, price, persons, hasDiscount, phoneNumbers);
        },
      );
    });
  }
    ...
    try {
      ...
      await billingService.purchaseWithCallback(productId, (
        ...
        if (success) {
          ...
          // 정기결제(1인 구독권)와 일회성 구매(다인 구독권)를 다르게 처리
          try {
            // 로딩 다시 표시
            showDialog(
              context: context,
            ...
            var subscriptionResult =
                await SubscriptionService.validateGooglePlayPurchase(
                  ...
                  includeOwner: null, // 1인 구독권만 지원 (구매하기는 1인만 가능)
                );

            // 구독권 생성 결과 확인
            if (subscriptionResult == null ||
                (subscriptionResult?['error'] == true)) {
              // Fallback API로 처리된 경우는 성공으로 간주
              if (subscriptionResult != null &&
                  subscriptionResult['fallback'] == true) {
            ...
            // Fallback API로 생성된 경우 추가 처리
            if (subscriptionResult != null &&
                subscriptionResult['fallback'] == true) {
              print('✅ Fallback API로 구독권 생성 성공 - 데이터 추출');
              subscriptionResult = subscriptionResult['data'];
            }
            ...
            if (persons == 1) {
              // 1인 구독권의 경우 간단한 성공 메시지
              Navigator.of(context).pop(); // 로딩 닫기
              _showSuccessDialog('1인 구독권 결제가 완료되었습니다!\n구독권이 활성화되었습니다.');
            } else {
              // 다인 구독권의 경우 쿠폰코드 처리
          ...
          try {
            ...
            var subscriptionResult =
                await SubscriptionService.validateGooglePlayPurchase(
                  ...
                  productId: productId,
                  purchaseToken: purchaseToken,
                  includeOwner: includeMyself, // 사용자가 선택한 값 전달
                );
            ...
            if (subscriptionResult == null ||
              ...
              if (subscriptionResult != null &&
                  subscriptionResult['fallback'] == true) {
                print('✅ Fallback API로 구독권 생성 성공 (선물하기)');
                subscriptionResult = subscriptionResult['data'];
              } else {
                Navigator.of(context).pop(); // 로딩 닫기
                final errorMessage =
            ...
            if (subscriptionResult != null &&
                subscriptionResult['fallback'] == true) {
              print('✅ Fallback API로 구독권 생성 성공 - 데이터 추출 (선물하기)');
              subscriptionResult = subscriptionResult['data'];
            }
  ...
  Future<void> _sendGiftToKakao(
    ...
    try {
      ...
      if (availableInviteCodes.isNotEmpty) {
        ...
        // 첫 번째 쿠폰코드만 카카오톡으로 전송 (모든 사람이 같은 코드 사용)
        String sharedInviteCode = availableInviteCodes.first;

        print('카카오톡으로 쿠폰코드 전송 시도: $sharedInviteCode');

        // 카카오톡 전송 시작 - 모달 표시하지 않음
        final success = await SubscriptionService.shareInviteCodeToKakao(
          sharedInviteCode,
          persons,
        );

        print('카카오톡 전송 결과: 성공=$success');

        // 카카오톡 전송 완료 후 잠시 대기 (앱이 포그라운드로 돌아올 시간 확보)
        ...
        if (success) {
          ...
          _showPurchaseCompletionDialog(
            title: '3인 구독권',
            price: '7,500원',
            originalPrice: '10,500원',
            discount: '총 3,500원 할인 받았어요',
            inviteCode: sharedInviteCode,
            persons: persons,
            isSuccess: true,
          );
