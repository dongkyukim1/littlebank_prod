...
import '../detail/chat_room_menu_screen.dart';
    ...
    try {
      ...
      if (mounted) {
        ...
        if (!isDuplicate) {
          ...
          _scrollToBottom();
          print('새 그룹 메시지 추가됨: ${message.content}');
        } else {
          print('중복 그룹 메시지 무시됨: ${message.content}');
        }
                ...
                Navigator.push(
                  ...
                  if (result != null && result['action'] == 'update_room_name') {
                    ...
                    if (success && newName != null) {
                      // 그룹 채팅방 이름이 변경되었을 때 상단 제목 업데이트
                      setState(() {
                        _roomName = newName;
                      });
                      print('✅ 그룹 채팅 화면 제목 업데이트: $newName');
                    }
                    
                    // 채팅 목록으로 결과 전달
                    Navigator.pop(context, result);
                  } else if (result != null && result['action'] == 'leave_room') {
                    // 나가기 결과 처리
                    Navigator.pop(context, result);
                  }
                });
