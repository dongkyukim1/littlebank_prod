...
class _PinInputBottomSheetState extends State<PinInputBottomSheet> {
  ...
  Future<void> _confirmPin() async {
    ...
    try {
      ...
      // PIN 검증 성공 시 출금 처리 및 완료 화면으로 이동
      print('🚀 [PIN SUCCESS] PIN 검증 성공 - 조건 체크 시작');
      print('🚀 [PIN SUCCESS] selectedBank: ${widget.selectedBank}');
      print('🚀 [PIN SUCCESS] accountNumber: ${widget.accountNumber}');
      print('🚀 [PIN SUCCESS] amount: ${widget.amount}');
      print('🚀 [PIN SUCCESS] fee: ${widget.fee}');
      print('🚀 [PIN SUCCESS] netAmount: ${widget.netAmount}');
      ...
      if (widget.selectedBank != null &&
        ...
        print('🚀 [PIN SUCCESS] 모든 조건 통과 - 아이 포인트 꺼내기 API 호출 시작');
        ...
        print('🔥 [간단한 DEBUG] 아이 포인트 꺼내기 API 성공');
        print('🔥 [간단한 DEBUG] response: $response');
        ...
        // API 응답에서 남은 포인트 가져오기 - 매우 간단한 방법
        int remainingPoint = response['remainingPoint'] ?? 0;
        print(
          '🔥 [간단한 DEBUG] response[remainingPoint]: ${response['remainingPoint']}',
        );
        print('🔥 [간단한 DEBUG] 파싱된 remainingPoint: $remainingPoint');

        // 바텀시트 닫기
        Navigator.pop(context);

        // 완료 화면으로 이동
        print('🔥 [간단한 DEBUG] TransferCompleteScreen으로 이동');
        print('🔥 [간단한 DEBUG] 전달할 remainingPoint: $remainingPoint');
          ...
          MaterialPageRoute(
            builder:
                (context) => TransferCompleteScreen(
                  ...
                  receiverAccount: widget.receiverPhone!, // 전화번호를 계좌번호로 사용
                  senderAccount: widget.senderName ?? '나',
                  amount: widget.amount!,
                  remainingBalance: remainingPoint,
                ),
          ),
        );
      } else {
        // 기존 로직 유지 (출금이 아닌 다른 용도로 사용될 때)
        print('🚨 [PIN ERROR] 조건 실패 - onPinConfirm 호출');
        print('🚨 [PIN ERROR] selectedBank: ${widget.selectedBank}');
        print('🚨 [PIN ERROR] accountNumber: ${widget.accountNumber}');
        print('🚨 [PIN ERROR] amount: ${widget.amount}');
        print('🚨 [PIN ERROR] fee: ${widget.fee}');
        print('🚨 [PIN ERROR] netAmount: ${widget.netAmount}');
