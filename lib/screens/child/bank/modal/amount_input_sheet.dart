...
class _AmountInputSheetState extends State<AmountInputSheet> {
  ...
  void _goToConfirmStep() async {
    ...
    // 현재 사용자 정보 가져오기
    String senderName = '보낸 사람';
    try {
      final userInfo = await AuthService.getUserInfo();
      senderName = userInfo['name'] ?? '보낸 사람';
    } catch (e) {
    ...
    showModalBottomSheet(
      ...
      builder:
          (context) => ConfirmSendBottomSheet(
            senderName: senderName, // 보낸 사람 정보 추가
            ...
            receiverId: widget.receiverId, // 받을 사람의 userId 전달
            fee: _getFee(),
            ...
            onConfirm: () {
              ...
              Navigator.pop(context);
              // 여기에 실제 송금 로직 추가
              print('송금 확인됨: ${widget.userName}님에게 $_inputAmount원');
            },
            onClose: () {
              Navigator.pop(context);
            },
