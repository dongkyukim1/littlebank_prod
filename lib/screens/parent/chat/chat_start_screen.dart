...
class _ChatStartScreenState extends State<ChatStartScreen> {
    ...
    try {
      ...
      try {
        if (_displayName.isNotEmpty) {
          ...
          final feedCount = await FeedService.getUserFeedCountByName(
            _displayName,
          );
  ...
  void _showMoreMenu(BuildContext context) {
    ...
    // 친구 관계 확인: 서버에서 찾은 friendId 또는 위젯에서 전달받은 friendId 사용
    int? effectiveFriendId = friendId ?? widget.friendId;
    bool canBlock = effectiveFriendId != null;

    print(
      '더보기 메뉴 표시: 서버friendId=$friendId, 위젯friendId=${widget.friendId}, 최종friendId=$effectiveFriendId, 차단 상태=$_isBlocked (전달받은 상태: ${widget.isBlocked})',
    );
                      ...
                      children: [
                        GestureDetector(
                          onTap: () {
                            Navigator.pop(context);
                            if (_isBlocked) {
                              _showUnblockFriendDialog(context, effectiveFriendId);
                            } else {
                              _showBlockFriendDialog(context, effectiveFriendId);
                            }
                          },
                          child: Container(
                            width: double.infinity,
                        ...
                        GestureDetector(
                          onTap: () {
                            Navigator.pop(context);
                            _showDeleteFriendDialog(context, effectiveFriendId);
                          },
                          child: Container(
                            width: double.infinity,
                            child: Column(
  ...
  // 친구 삭제 확인 다이얼로그
  void _showDeleteFriendDialog(BuildContext context, int? effectiveFriendId) {
    int? friendId = effectiveFriendId ?? _findFriendIdFromUserInfo();
  ...
  // 친구 차단 확인 다이얼로그
  void _showBlockFriendDialog(BuildContext context, int? effectiveFriendId) {
    int? friendId = effectiveFriendId ?? _findFriendIdFromUserInfo();
  ...
  // 친구 차단 해제 확인 다이얼로그
  void _showUnblockFriendDialog(BuildContext context, int? effectiveFriendId) {
    int? friendId = effectiveFriendId ?? _findFriendIdFromUserInfo();
