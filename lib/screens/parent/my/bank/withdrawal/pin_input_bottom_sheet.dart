...
import '../parent_withdrawal_complete_screen.dart';
    ...
    try {
      ...
      if (widget.selectedBank != null &&
        ...
        if (widget.isWithdrawal) {
          // 자기 자신에게 출금하는 경우
          print('PIN 검증 성공 - 자기 자신에게 포인트 출금 처리');
          print('API 호출 파라미터 확인:');
          print('- exchangeAmount: ${widget.netAmount}');

          // 실제 자기 자신에게 출금하는 API 호출
          final userInfo = await AuthService.getUserInfo();
          final userId = userInfo['userId'];

          response = await PaymentService.refundPointsParent(
            exchangeAmount: widget.netAmount!,
            depositTargetUserId: userId, // 자기 자신의 userId
          );
        } else {
          // 다른 사람에게 송금하는 경우 (기존 로직)
          print('PIN 검증 성공 - 부모 포인트 꺼내기 API 호출 시작');
          print('API 호출 파라미터 확인:');
          print('- exchangeAmount: ${widget.netAmount}');
        ...
        print('부모 포인트 꺼내기 API 성공: $response');

        // API 응답에서 남은 포인트 추출
        final remainingPoint = response['remainingPoint'] ?? 0;
        ...
        Navigator.pushReplacement(
          ...
          MaterialPageRoute(
            builder:
                (context) => ParentWithdrawalCompleteScreen(
                  ...
                  remainingPoint: remainingPoint, // API에서 받은 남은 포인트 전달
                ),
          ),
        );
      } else {
