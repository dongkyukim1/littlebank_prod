...
class ParentFamilyManagementScreen extends StatefulWidget {
  const ParentFamilyManagementScreen({super.key});

  @override
  State<ParentFamilyManagementScreen> createState() =>
      _ParentFamilyManagementScreenState();
}

class _ParentFamilyManagementScreenState
    extends State<ParentFamilyManagementScreen> {
  // 사용자 정보 및 로딩 상태
    ...
    return GestureDetector(
      ...
      child: Stack(
        children: [
          ...
          // 프로필 정보 카드 (하단)
          Positioned(
            bottom: 40, // 32에서 40으로 변경하여 위로 올림
            left: 12,
            right: 12,
            child: Container(
  ...
  Widget _buildFamilySection() {
    ...
    final int totalMembers =
        displayMembers.length + (_pendingInvites?.length ?? 0);

    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        // 헤더 섹션 (제목과 가족 추가하기 버튼)
        Padding(
        ...
        const SizedBox(height: 32), // 타이틀과 멤버 카드 사이 간격
        // 가족 멤버 목록 (가로 스크롤)
        SizedBox(
          height: totalMembers > 0 ? 200 : 0, // 멤버가 있을 때만 높이 지정
          child:
              totalMembers > 0
                  ? ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      if (index < displayMembers.length) {
                        ...
                        final String role = member['role'] ?? 'CHILD';
                        final String? profileImg = member['profileImagePath'];

                        // 화면 너비에 따라 카드 너비 계산
                        double screenWidth = MediaQuery.of(context).size.width;
                        double cardWidth =
                            (screenWidth - (32 + 12)) / 2; // 패딩과 간격 고려

                        return Container(
                          width: cardWidth,
                          margin: EdgeInsets.only(
                            right: index != totalMembers - 1 ? 16 : 0,
                          ),
                      ...
                      // 초대 중인 멤버 목록 표시
                        ...
                        final pendingIndex = index - displayMembers.length;
                        if (pendingIndex < (_pendingInvites?.length ?? 0)) {
                          final invite = _pendingInvites![pendingIndex];
                          ...
                          return Container(
                            width: cardWidth,
                            margin: EdgeInsets.only(
                              right: index != totalMembers - 1 ? 12 : 0,
                            ),
        ...
        const SizedBox(height: 50), // 멤버 카드와 버튼 사이 간격
        // 연락처로 초대하기 버튼
        Padding(
          padding: const EdgeInsets.symmetric(horizontal: 16),
          child: GestureDetector(
        ...
        const SizedBox(height: 40), // 버튼과 안내 카드 사이 간격
        // 안내 카드들
        Padding(
          padding: const EdgeInsets.symmetric(horizontal: 16),
          child: Column(
            children: [
              _buildInfoCard(
                ...
                '리틀뱅크에서는 자녀에게 미션을 제공하고 보상을 줄 수 있어요. 자녀의 금융 교육을 책임지세요.',
              ),
  ...
  // 가족 멤버 삭제 확인 대화상자
  void _showDeleteFamilyMemberDialog(
    BuildContext context,
    String memberName,
    int familyMemberId,
  ) {
        ...
        return AlertDialog(
          ...
          content: Column(
            ...
            children: [
              ...
              SizedBox(height: 12),
              Text(
                '⚠️ 주의: 가족 멤버에서 삭제하면 더 이상 이 멤버와 관련된 기능을 사용할 수 없게 됩니다.',
                style: TextStyle(
                  fontSize: 13,
                  fontFamily: 'Pretendard-Light',
                  color: Color(0xFFFF6B6B),
          ...
          actions: [
            ...
            TextButton(
              ...
              onPressed: () {
                Navigator.of(context).pop();
                _deleteFamilyMember(familyMemberId);
              },
            ),
          ],
        );
  ...
  // 가족 멤버 삭제 메서드
  Future<void> _deleteFamilyMember(int familyMemberId) async {
    try {
      setState(() {
        _isLoading = true;
      });

      final result = await FamilyService.deleteFamilyMember(familyMemberId);

      if (result['success']) {
        // 삭제 성공
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('가족 멤버가 삭제되었습니다'),
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(result['message'] ?? '가족 멤버 삭제에 실패했습니다'),
            backgroundColor: Colors.red,
          ),
        );
      }
  ...
  Widget _buildPendingInviteCard(Map<String, dynamic> invite) {
    ...
    final int? familyMemberId = invite['familyMemberId']; // 가족 멤버 ID 추출
    ...
    if (invitedDate.isNotEmpty) {
      try {
        ...
        print(
          '초대 시간 - UTC: $utcDateTime, 한국: $koreanDateTime, 현재: $now, 차이: $difference',
        );
      } catch (e) {
        print('날짜 파싱 오류: $e');
        displayDate = '최근에 초대됨';
                ...
                Container(
                  ...
                  child: Column(
                    children: [
                      // 닉네임 변경 버튼
                      ListTile(
                        leading: Icon(Icons.edit, color: Color(0xFF146AFF)),
                        title: Text(
                          '별명 변경하기',
                          style: TextStyle(
                            fontSize: 14,
                        ...
                        onTap: () {
                          ...
                          if (familyMemberId != null) {
                            ...
                            ScaffoldMessenger.of(context).showSnackBar(
                              SnackBar(content: Text('가족 멤버 정보를 찾을 수 없습니다')),
                            );
                          }
                        },
                      ),

                      // 내 계정인 경우에만 가족 그룹 나가기 버튼 표시
                      if (_isMyAccount(familyMemberId))
                        ListTile(
                          leading: Icon(
                            Icons.exit_to_app,
                            color: Color(0xFFFF6B6B),
                          ...
                          onTap: () {
                            // 현재 다이얼로그 닫기
                            Navigator.of(context).pop();
                            // 가족 그룹 나가기 확인 대화상자 표시
                            if (familyMemberId != null) {
                              _showLeaveFamilyConfirmDialog(
                                context,
                                memberName,
                                familyMemberId,
                              );
  ...
  // 가족 멤버 추방 확인 대화상자
  void _showForceOutConfirmDialog(
    BuildContext context,
    String memberName,
    int familyMemberId,
  ) {
        ...
        return AlertDialog(
          ...
          content: Column(
            ...
            children: [
              ...
              SizedBox(height: 12),
              Text(
                '⚠️ 주의: 가족에서 내보내면 더 이상 $memberName님과 관련된 기능을 사용할 수 없게 됩니다.',
                style: TextStyle(
                  fontSize: 13,
                  fontFamily: 'Pretendard-Light',
                  color: Color(0xFFFF6B6B),
          ...
          actions: [
            ...
            TextButton(
              ...
              onPressed: () {
                Navigator.of(context).pop();
                _forceOutFamilyMember(familyMemberId);
              },
            ),
          ],
        );
  ...
  // 가족 멤버 추방 메서드
  Future<void> _forceOutFamilyMember(int familyMemberId) async {
    try {
      setState(() {
        _isLoading = true;
      });

      final result = await FamilyService.forceOutFamilyMember(familyMemberId);

      if (result['success']) {
        // 추방 성공
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('가족 멤버가 성공적으로 제외되었습니다'),
            backgroundColor: Colors.green,
          ),
        );
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(result['message'] ?? '가족 멤버 제외에 실패했습니다'),
            backgroundColor: Colors.red,
          ),
        );
      }
                      ...
                      TextField(
                        ...
                        decoration: InputDecoration(
                          ...
                          focusedBorder: OutlineInputBorder(
                            borderRadius: BorderRadius.circular(8),
                            borderSide: BorderSide(color: Color(0xFF146AFF)),
                          ),
                        ),
                      ),
                      SizedBox(height: 6),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: RefreshIndicator(
        ...
        child:
            _isLoading && (_userInfo == null || _familyMembers == null)
                ? const Center(
                ...
                : ((_familyMembers != null && _familyMembers!.isNotEmpty) ||
                    (_pendingInvites != null && _pendingInvites!.isNotEmpty))
                ...
                : // 가족 멤버와 초대 중인 멤버가 모두 없을 때의 전체 화면
                SingleChildScrollView(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
